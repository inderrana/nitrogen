class NewTabHomepage {
    ...
    async fetchWeatherByCity(cityName, autoRefresh = false) {
        ...
        try {
            // Use Open-Meteo (European weather service) - no API key required, GDPR compliant
            // First get coordinates for the city using their geocoding API with timeout
            ...
            if (content.links && content.links.length > 0) {
                ...
                if (linksGrid) {
                    ...
                    content.links.forEach(linkData => {
                        this.createLinkElement(linksGrid, linkData.url, linkData.name, linkData.icon);
                    });
    ...
    initDraggableWidgets() {
        ...
        widgets.forEach(widget => {
            ...
            handle.addEventListener('mousedown', (e) => {
                ...
                // Don't drag if clicking directly on the drag icon (let it be just visual)
            ...
            document.addEventListener('mouseup', () => {
                if (isDragging) {
                    ...
                    this.saveWidgetPosition(widgetId, currentX, currentY);
                }
            });
        });
    ...
    loadWidgetPosition(widget, widgetId) {
        ...
        if (savedPositions && savedPositions[widgetId]) {
            const { x, y } = savedPositions[widgetId];
        ...
        if (this.userProfileManager && this.userProfileManager.isUserLoggedIn()) {
            ...
            if (user) {
                ...
                if (password) {
                    ...
                    this.log('[WIDGETS] Widget position saved to profile:', widgetId);
                }
            }
        }
    }
    ...
    populateLinksManager() {
        ...
        // Get all current links from the grid
        const linksGrid = document.getElementById('linksGrid');
        if (!linksGrid) {
            console.warn('[CUSTOMIZE] Links grid not found');
            return;
        }

        const links = Array.from(linksGrid.querySelectorAll('.link-item')).map((item, index) => {
            const link = item.querySelector('.link-card');
            const nameEl = link.querySelector('.link-name');
            const iconEl = link.querySelector('.link-icon');
            return {
    ...
    saveLinkEdit(index) {
        ...
        // Update the actual link in the grid
        const linksGrid = document.getElementById('linksGrid');
    ...
    applyNewLinkOrder() {
        ...
        // Clear and rebuild the grid with the new order
        linksGrid.innerHTML = '';
